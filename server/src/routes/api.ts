...
router.post(
  "/subscribe",
  async (
    req: express.Request<{}, {}, SubscriptionRequestBody>,
...
router.get(
  "/confirm/:token",
  async (req: express.Request<{ token: string }>, res: express.Response) => {
...
router.get(
  ...
  async (req: express.Request<{ token: string }>, res: express.Response) => {
    const { token } = req.params;
    try {
      const deleted = await db<Subscription>("subscriptions")
        .where({ token })
        .del();
      if (deleted) {
