...
class TestWeatherProvider extends BaseWeatherProvider {
  ...
  constructor(logger: Logger, name: string = "TestProvider", shouldFail: boolean = false) {
    super(name, logger);
...
describe("BaseWeatherProvider", () => {
  ...
  it("should fetch weather data and log response", async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith("Weather data fetched successfully", {
  ...
  it("should pass request to next provider when current provider fails", async () => {
    const failingProvider = new TestWeatherProvider(mockLogger, "FailingProvider", true);
    const workingProvider = new TestWeatherProvider(mockLogger, "WorkingProvider", false);
    ...
    expect(mockLogger.error).toHaveBeenCalledWith("Error fetching weather data from provider", {
  ...
  it("should throw error if all providers in chain fail", async () => {
    const failingProvider1 = new TestWeatherProvider(mockLogger, "FailingProvider1", true);
    const failingProvider2 = new TestWeatherProvider(mockLogger, "FailingProvider2", true);
    ...
    expect(mockLogger.error).toHaveBeenNthCalledWith(1, "Error fetching weather data from provider", {
    ...
    expect(mockLogger.error).toHaveBeenNthCalledWith(2, "Error fetching weather data from provider", {
