...
test.describe("Weather Subscription SPA", () => {
  ...
  test("Get weather for valid city", async ({ page }) => {
    ...
    await Promise.all([
      page.waitForResponse(response => response.url().includes('/weather')),
      page.click('section:has(h4:text("Current Weather")) button:has-text("Show")')
    ]);
        
    await expect(page.locator('section:has(h4:text("Current Weather")) li:has-text("Temperature")')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('section:has(h4:text("Current Weather")) li:has-text("Humidity")')).toBeVisible();
    await expect(page.locator('section:has(h4:text("Current Weather")) li:has-text("Description")')).toBeVisible();
    });
